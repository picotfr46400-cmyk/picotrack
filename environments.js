...
// ══ PicoTrack — Client API sécurisé v23 (proxy serveur) ══
// Le navigateur ne contacte plus directement /rest/v1 de Supabase.
// Les requêtes passent par /api/supabase-proxy afin de masquer les endpoints techniques
// dans l'onglet Network et de préparer la migration de la logique métier côté serveur.
const SUPA_URL = '';
const SUPA_KEY = '';
const _supa = null;

function _ptApiToken() {
  try { return localStorage.getItem('pt_access_token') || ''; } catch { return ''; }
}
function _ptStoreSession(session) {
  try {
    if (!session) return;
    localStorage.setItem('pt_access_token', session.access_token || '');
    localStorage.setItem('pt_refresh_token', session.refresh_token || '');
    localStorage.setItem('pt_session_payload', JSON.stringify(session || {}));
  } catch {}
}
function _ptClearSession() {
  try {
    localStorage.removeItem('pt_access_token');
    localStorage.removeItem('pt_refresh_token');
    localStorage.removeItem('pt_session_payload');
    localStorage.removeItem('pt_pc_session');
  } catch {}
}
function _ptReadSession() {
  try {
    const raw = localStorage.getItem('pt_session_payload');
    return raw ? JSON.parse(raw) : null;
  } catch { return null; }
}
function _ptEncodePayload(value) {
  const str = typeof value === 'string' ? value : JSON.stringify(value || {});
  return btoa(unescape(encodeURIComponent(str)));
}

// ── Config active ──
function _getSupaConfig() { return { url: '/api', key: '' }; }
...
// ── Token auth ──
async function _getAuthHeader() { return _ptApiToken(); }
...
// ── sbFetch : requête via proxy serveur PicoTrack ──
async function sbFetch(path, options = {}) {
  ...
  const res = await fetch('/api/supabase-proxy', {
    ...
    headers,
    body: JSON.stringify({
      p: _ptEncodePayload(path),
      m: options.method || 'GET',
      b: options.body ? _ptEncodePayload(options.body) : null,
  ...
  if (!res.ok) {
    ...
    updateSupabaseStatusUI('offline', `API ${res.status}`);
  ...
  updateSupabaseStatusUI('online', 'API connectée · ' + _ptLastSyncLabel);
...
async function sbFunction(functionName, payload = {}) {
  const token = await _getAuthHeader();
  const res = await fetch('/api/function-proxy', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(token ? { 'Authorization': `Bearer ${token}` } : {})
    },
    body: JSON.stringify({ f: _ptEncodePayload(functionName), p: _ptEncodePayload(payload) })
  });
  const text = await res.text();
  let json = null;
  try { json = text ? JSON.parse(text) : null; } catch { json = { message: text }; }
  if (!res.ok) throw new Error(json?.error || json?.message || `Function proxy ${res.status}`);
  return json;
}
...
async function ptSignIn(email, password) {
  ...
  if (!res.ok || data.error) throw new Error(data.error || 'Connexion impossible');
  _ptStoreSession(data.session || data);
  return data;
}

async function ptSignOut() {
  const token = _ptApiToken();
  try {
...
async function ptGetSession() {
  ...
  if (!session.user && session.access_token) {
    try {
      ...
      if (res.ok && data.user) { session.user = data.user; _ptStoreSession(session); }
    } catch {}
  }
  return session;
}
...
if(!payload.pdfData) delete payload.pdfData;
