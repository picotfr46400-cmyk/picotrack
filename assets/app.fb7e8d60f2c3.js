...
;/* PicoTrack secure API client v27 - no direct Supabase in browser */
const PT_CLIENT_CODE = 'client';
const PT_ENVIRONMENT_CODE = 'runtime';
window.PT_CLIENT_CODE = PT_CLIENT_CODE;
window.PT_ENVIRONMENT_CODE = PT_ENVIRONMENT_CODE;
const PT_SESSION_KEY = 'pt_auth_session_proxy_v27';
let _ptSupabaseOnline = false;
let _ptLastSyncLabel = '—';
function _ptB64Encode(str){
  try { return btoa(unescape(encodeURIComponent(String(str||'')))); }
  catch { return btoa(String(str||'')); }
}
function _ptB64Decode(str){
  try { return decodeURIComponent(escape(atob(String(str||'')))); }
  catch { return atob(String(str||'')); }
}
function _ptReadSession(){
  try { return JSON.parse(localStorage.getItem(PT_SESSION_KEY) || 'null'); } catch { return null; }
}
function _ptWriteSession(session){
  if(session) localStorage.setItem(PT_SESSION_KEY, JSON.stringify(session));
  else localStorage.removeItem(PT_SESSION_KEY);
}
async function _ptApiPost(url, payload){
...
async function _ptEnsureSession(){
  let s = _ptReadSession();
  if(!s) return null;
  const now = Math.floor(Date.now()/1000);
  if(s.expires_at && s.refresh_token && s.expires_at < now + 60){
    try {
      const out = await _ptApiPost('/api/auth', { action:'refresh', refresh_token:s.refresh_token });
      if(out && out.session){ _ptWriteSession(out.session); s = out.session; }
    } catch(e){ console.warn('[Auth] refresh impossible:', e.message); _ptWriteSession(null); return null; }
  }
  return s;
}
...
async function _getAuthHeader(){
  const s = await _ptEnsureSession();
...
async function sbFetch(path, options = {}) {
  ...
  const json = await _ptApiPost('/api/data', payload);
  _ptSupabaseOnline = true;
  _ptLastSyncLabel = new Date().toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit', second:'2-digit' });
  updateSupabaseStatusUI('online', 'Connecté · ' + _ptLastSyncLabel);
  return json?.data ?? [];
}
async function sbFunction(functionName, payload = {}) {
  const token = await _getAuthHeader();
  return _ptApiPost('/api/function', { f:_ptB64Encode(functionName), payload, token });
}
...
function _getTenantId(){ return null; }
function _withTenant(data) {
  const env = _getEnvironmentCode();
  const clean = (d) => { const out = { ...(d || {}) }; delete out.tenant_id; if (!out.environment_code) out.environment_code = env; return out; };
  return Array.isArray(data) ? data.map(clean) : clean(data);
}
async function hashPassword(pass){ return String(pass || ''); }
async function ptSignIn(email, password) {
  const out = await _ptApiPost('/api/auth', { action:'signIn', email, password });
  if(out && out.session){ _ptWriteSession(out.session); return { session: out.session, user: out.session.user }; }
  throw new Error('Connexion impossible');
}
async function ptSignOut() {
  try { await _ptApiPost('/api/auth', { action:'signOut', token:(await _getAuthHeader()) }); } catch {}
  _ptWriteSession(null);
  sessionStorage.removeItem('pt_active_env');
  localStorage.removeItem('pt_pc_session');
  location.reload();
}
async function ptGetSession() { return await _ptEnsureSession(); }
async function ptGetCurrentUser() {
  ...
  try {
    ...
    const p = rows[0];
    return { id:session.user.id, email:session.user.email, role:p.role, roles:p.roles || [], environment_code:p.environment_code, license_type:p.license_type || (p.role === 'super_admin' ? 'super_admin' : 'supervision'), active:p.active === true, resolved_permissions:p.resolved_permissions || {} };
  } catch (e) { console.warn('[Auth] profil introuvable:', e.message); return null; }
}
async function checkSupabaseConnection() {
  try { updateSupabaseStatusUI('syncing', 'Vérification…'); await sbFetch('forms?select=id&limit=1'); updateSupabaseStatusUI('online', 'Connecté'); return true; }
  catch(e){ updateSupabaseStatusUI('offline', 'Connexion impossible'); console.warn('[PicoTrack] connexion impossible:', e.message); return false; }
}
const _supa = { auth: {
  getSession: async () => ({ data: { session: await ptGetSession() }, error:null }),
  signInWithPassword: async ({ email, password }) => ({ data: await ptSignIn(email, password), error:null }),
  signOut: async () => { await ptSignOut(); return { error:null }; },
  updateUser: async ({ password }) => {
    const token = await _getAuthHeader();
    const out = await _ptApiPost('/api/auth', { action:'updateUser', password, token });
    if(out.session) _ptWriteSession(out.session);
    return { data:out, error:null };
  },
  exchangeCodeForSession: async (code) => {
    const out = await _ptApiPost('/api/auth', { action:'exchangeCode', code });
    if(out.session) _ptWriteSession(out.session);
    return { data:out, error:null };
  }
}};
