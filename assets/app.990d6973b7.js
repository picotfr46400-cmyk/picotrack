...
const C = {
    em: "#2563EB", emd: "#1D4ED8", eml: "#EFF6FF", em2: "#BFDBFE",
    ok: "#10B981", okL: "#ECFDF5", danger: "#EF4444", dangerL: "#FEF2F2",
...
function detect(l) { for (const [re, t] of SMART_DETECT)
    if (re.test(l))
...
function getVlds(t) {
    ...
    if (["text"].includes(t))
        return [...b, { n: "Min caractères", v: true }, { n: "Max caractères", v: true }, { n: "Lettres uniquement", v: false }, { n: "Chiffres uniquement", v: false }, { n: "Email", v: false }, { n: "Expression régulière", v: true }];
    if (t === "textarea")
        return [...b, { n: "Min caractères", v: true }, { n: "Max caractères", v: true }];
    if (t === "number")
        return [...b, { n: "Valeur min", v: true }, { n: "Valeur max", v: true }];
    if (["select", "multi"].includes(t))
        return [...b, { n: "Nb sélections min", v: true }, { n: "Nb sélections max", v: true }];
    if (t === "date")
        return [...b, { n: "Date min", v: true }, { n: "Date max", v: true }];
    if (t === "time")
        return [...b, { n: "Heure min", v: true }, { n: "Heure max", v: true }];
    if (["photo", "file"].includes(t))
        return [...b, { n: "Nb fichiers min", v: true }, { n: "Nb fichiers max", v: true }];
    return b;
}
function Tog({ on, cb }) {
    return (React.createElement("div", { onClick: cb, style: { width: 32, height: 18, borderRadius: 9, background: on ? C.em : C.bd, cursor: "pointer", position: "relative", transition: "background .2s", flexShrink: 0 } },
        React.createElement("div", { style: { width: 13, height: 13, borderRadius: "50%", background: "#fff", position: "absolute", top: 2.5, left: on ? 16 : 2.5, transition: "left .2s", boxShadow: "0 1px 3px rgba(0,0,0,.2)" } })));
}
function Badge({ tid, xs }) {
    const t = TYPES.find(x => x.id === tid);
    if (!t)
        return null;
    return (React.createElement("span", { style: { display: "inline-flex", alignItems: "center", gap: 3, padding: xs ? "2px 6px" : "3px 8px", borderRadius: 20, fontSize: xs ? 10 : 11, fontWeight: 700, background: t.clr + "18", color: t.clr, flexShrink: 0, whiteSpace: "nowrap" } },
        t.icon,
        " ",
        t.label));
}
function Preview({ f }) {
    ...
    if (f.type === "sep")
        return (React.createElement("hr", { style: { border: "none", borderTop: "1.5px solid " + C.bd, margin: "2px 0" } }));
    if (f.type === "text")
        return (React.createElement("input", { style: s, placeholder: f.ph || "Saisir...", readOnly: true }));
    if (f.type === "textarea")
        return (React.createElement("textarea", { style: Object.assign(Object.assign({}, s), { height: 60, resize: "none" }), placeholder: f.ph || "Saisir...", readOnly: true }));
    if (f.type === "number")
        return (React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center" } },
            React.createElement("div", { style: { width: 32, height: 32, border: "1.5px solid " + C.bd, borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", color: C.ts, fontSize: 16 } }, "\u2212"),
            React.createElement("input", { type: "number", style: Object.assign(Object.assign({}, s), { width: 76, textAlign: "center" }), defaultValue: 0, readOnly: true }),
            React.createElement("div", { style: { width: 32, height: 32, background: C.em, borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 16 } }, "+")));
    if (f.type === "checkbox")
        return (React.createElement("div", { style: { display: "inline-flex", alignItems: "center", gap: 9, padding: "8px 12px", border: "1.5px solid " + C.bd, borderRadius: 8, background: C.bg } },
            React.createElement("input", { type: "checkbox", style: { width: 15, height: 15, accentColor: C.em }, readOnly: true }),
            React.createElement("span", { style: { fontSize: 13, color: C.tl } }, "Cocher si applicable")));
    if (f.type === "select")
        return (React.createElement("select", { style: Object.assign(Object.assign({}, s), { appearance: "none", cursor: "default" }) },
            React.createElement("option", null, "S\u00E9lectionner..."),
            (f.opts || []).map(o => (React.createElement("option", { key: o }, o)))));
    if (f.type === "multi")
        return (React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 } }, (f.opts && f.opts.length ? f.opts : ["Option 1", "Option 2"]).map(o => (React.createElement("div", { key: o, style: { padding: "5px 13px", border: "1.5px solid " + C.bd, borderRadius: 20, fontSize: 12.5, color: C.tl, background: C.bg } }, o)))));
    if (f.type === "date")
        return (React.createElement("input", { type: "date", style: Object.assign(Object.assign({}, s), { width: 180, pointerEvents: "none" }), readOnly: true }));
    if (f.type === "time")
        return (React.createElement("input", { type: "time", style: Object.assign(Object.assign({}, s), { width: 140, pointerEvents: "none" }), readOnly: true }));
    if (f.type === "appointment")
        return (React.createElement("div", { style: { border: "1.5px solid " + C.bd, borderRadius: 10, background: "#fff", padding: 12 } },
            React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", marginBottom: 10 } },
                React.createElement("div", { style: { width: 36, height: 36, borderRadius: 10, background: "#DBEAFE", color: "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 } }, "\uD83D\uDCC5"),
                React.createElement("div", null,
                    React.createElement("div", { style: { fontSize: 13, fontWeight: 800, color: C.tx } }, "S\u00E9lection date + cr\u00E9neau"),
                    React.createElement("div", { style: { fontSize: 11, color: C.tl } },
                        "Capacit\u00E9 : ",
                        (f.parallelSlots || 1),
                        " place",
                        (f.parallelSlots || 1) > 1 ? "s" : "",
                        " par cr\u00E9neau"))),
            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 } }, ["08:00", "08:30", "09:00"].map((x, i) => React.createElement("div", { key: x, style: { padding: "7px 8px", border: "1px solid " + (i === 2 ? "#FCA5A5" : C.bd), borderRadius: 8, background: i === 2 ? "#FEF2F2" : "#F8FAFC", fontSize: 11, fontWeight: 800, color: i === 2 ? "#DC2626" : C.tm, textAlign: "center" } },
                x,
                i === 2 ? " · complet" : "")))));
    if (f.type === "photo")
        return (React.createElement("div", { style: { border: "2px dashed " + C.bd, borderRadius: 10, padding: 14, textAlign: "center", color: C.ts, fontSize: 13, background: C.bg } }, "\uD83D\uDCF7 Photo"));
    if (f.type === "location")
        return (React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 13px", border: "1.5px solid " + C.bd, borderRadius: 9, background: C.bg } },
            React.createElement("span", { style: { color: C.ts, fontSize: 13 } }, "\uD83D\uDCCD Non captur\u00E9"),
            React.createElement("span", { style: { padding: "3px 11px", border: "1.5px solid " + C.em, borderRadius: 20, color: C.em, fontSize: 12, fontWeight: 700 } }, "Capturer")));
    if (f.type === "sign")
        return (React.createElement("div", { style: { border: "2px dashed " + C.bd, borderRadius: 10, height: 58, display: "flex", alignItems: "center", justifyContent: "center", color: C.ts, fontSize: 13, background: C.bg } }, "\u270D Signature"));
    return null;
}
function OptsEd({ opts, onChange }) {
    return (React.createElement("div", { style: { marginTop: 10, padding: 10, background: C.bg, borderRadius: 8, border: "1px solid " + C.bd } },
        React.createElement("div", { style: { fontSize: 10, fontWeight: 700, color: C.ts, textTransform: "uppercase", letterSpacing: ".7px", marginBottom: 7 } }, "Options"),
        opts.map((o, i) => (React.createElement("div", { key: i, style: { display: "flex", gap: 5, marginBottom: 5 } },
            React.createElement("input", { value: o, onChange: e => { const n = [...opts]; n[i] = e.target.value; onChange(n); }, style: { flex: 1, border: "1.5px solid " + C.bd, borderRadius: 7, padding: "5px 9px", fontSize: 12.5, fontFamily: "inherit", outline: "none" }, onFocus: e => { e.target.style.borderColor = C.em; }, onBlur: e => { e.target.style.borderColor = C.bd; } }),
            React.createElement("button", { onClick: () => onChange(opts.filter((_, j) => j !== i)), style: { width: 26, height: 26, border: "1px solid #FCA5A5", borderRadius: 6, background: "#FEF2F2", color: "#EF4444", cursor: "pointer", flexShrink: 0 } }, "\u2715")))),
        React.createElement("button", { onClick: () => onChange([...opts, "Nouvelle option"]), style: { width: "100%", padding: 6, border: "1.5px dashed " + C.em2, borderRadius: 7, background: "transparent", color: C.em, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, "\uFF0B Ajouter")));
}
function TypePicker({ active, onSel }) {
    ...
    return (React.createElement("select", { value: active, onChange: e => onSel(e.target.value), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 9, padding: "9px 10px", fontSize: 12.5, fontWeight: 800, color: C.tm, background: "#fff", fontFamily: "inherit", outline: "none", marginBottom: 10 } }, cats.map(cat => (React.createElement("optgroup", { key: cat.id, label: cat.label }, TYPES.filter(t => t.cat === cat.id).map(t => (React.createElement("option", { key: t.id, value: t.id },
        t.icon,
        " ",
        t.label))))))));
}
function Adv({ f, upd }) {
    ...
    return (React.createElement("div", { style: { marginTop: 10, border: "1.5px solid " + C.bd, borderRadius: 10, overflow: "hidden" } },
        React.createElement("div", { style: { display: "flex", borderBottom: "1px solid " + C.bd, background: C.bg, padding: "0 6px", gap: 0, overflowX: "auto" } },
            React.createElement("button", { style: ts("gen"), onClick: () => setTab("gen") }, "G\u00E9n\u00E9ral"),
            ["text", "textarea"].includes(f.type) && React.createElement("button", { style: ts("trf"), onClick: () => setTab("trf") },
                "Transformateurs",
                tc > 0 && React.createElement("span", { style: { marginLeft: 4, padding: "1px 5px", borderRadius: 10, background: "#8b5cf6", color: "#fff", fontSize: 9 } }, tc)),
            React.createElement("button", { style: ts("vld"), onClick: () => setTab("vld") },
                "Validateurs",
                vc > 0 && React.createElement("span", { style: { marginLeft: 4, padding: "1px 5px", borderRadius: 10, background: C.em, color: "#fff", fontSize: 9 } }, vc)),
            React.createElement("button", { style: ts("cnd"), onClick: () => setTab("cnd") },
                "Conditions",
                cc > 0 && React.createElement("span", { style: { marginLeft: 4, padding: "1px 5px", borderRadius: 10, background: "#f59e0b", color: "#fff", fontSize: 9 } }, cc)),
            React.createElement("button", { style: ts("dsp"), onClick: () => setTab("dsp") }, "Affichage")),
        React.createElement("div", { style: { padding: 14, background: "#fff" } },
            tab === "gen" && (React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 12 } },
                React.createElement("div", null,
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Nom technique"),
                    React.createElement("input", { value: f.key || slugKey(f.label), onChange: e => upd(f.id, { key: e.target.value }), placeholder: "ex : numero_bl", style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5, fontFamily: "inherit", outline: "none" } })),
                !["sep", "titre", "image", "photo", "sign", "location", "file"].includes(f.type) && (React.createElement("div", null,
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Valeur par d\u00E9faut"),
                    ["select", "multi"].includes(f.type) ? (React.createElement("select", { value: f.def || "", onChange: e => upd(f.id, { def: e.target.value }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5, fontFamily: "inherit", outline: "none", background: "#fff" } },
                        React.createElement("option", { value: "" }, "Aucune valeur par d\u00E9faut"),
                        (f.opts || []).map((o, i) => React.createElement("option", { key: i, value: o }, o)))) : (React.createElement("input", { value: f.def || "", onChange: e => upd(f.id, { def: e.target.value }), placeholder: "Pr\u00E9-remplissage c\u00F4t\u00E9 PAD", style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5, fontFamily: "inherit", outline: "none" } })))),
                ["text", "textarea", "number"].includes(f.type) && (React.createElement("div", null,
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Placeholder"),
                    React.createElement("input", { value: f.ph || "", onChange: e => upd(f.id, { ph: e.target.value }), placeholder: "Ex : Saisir une valeur...", style: { width: "100%", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5, fontFamily: "inherit", outline: "none" }, onFocus: e => { e.target.style.borderColor = C.em; }, onBlur: e => { e.target.style.borderColor = C.bd; } }))),
                f.type === "number" && (React.createElement("div", { style: { border: "1.5px solid " + C.bd, borderRadius: 10, padding: 12, background: C.bg } },
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 900, color: C.tm, marginBottom: 10 } }, "Param\u00E8tres nombre"),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 } },
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Pr\u00E9cision"),
                            React.createElement("input", { type: "number", min: "0", value: f.precision || 0, onChange: e => upd(f.id, { precision: +e.target.value || 0 }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5 } })),
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Pas"),
                            React.createElement("input", { type: "number", value: f.step || 1, onChange: e => upd(f.id, { step: e.target.value }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5 } }))),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center", marginTop: 10 } },
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Valeur minimum"),
                            React.createElement("input", { disabled: !f.minEnabled, type: "number", value: f.minValue || "", onChange: e => upd(f.id, { minValue: e.target.value }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5, opacity: f.minEnabled ? 1 : .55 } })),
                        React.createElement(Tog, { on: !!f.minEnabled, cb: () => upd(f.id, { minEnabled: !f.minEnabled }) })),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center", marginTop: 10 } },
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Valeur maximum"),
                            React.createElement("input", { disabled: !f.maxEnabled, type: "number", value: f.maxValue || "", onChange: e => upd(f.id, { maxValue: e.target.value }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5, opacity: f.maxEnabled ? 1 : .55 } })),
                        React.createElement(Tog, { on: !!f.maxEnabled, cb: () => upd(f.id, { maxEnabled: !f.maxEnabled }) })))),
                f.type === "appointment" && (React.createElement("div", { style: { border: "1.5px solid #BFDBFE", borderRadius: 10, padding: 12, background: "#EFF6FF" } },
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 900, color: "#1D4ED8", marginBottom: 10 } }, "\uD83D\uDCC5 Param\u00E8tres rendez-vous"),
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 } },
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Dur\u00E9e cr\u00E9neau (min)"),
                            React.createElement("input", { type: "number", min: "5", step: "5", value: f.slotDuration || 30, onChange: e => upd(f.id, { slotDuration: +e.target.value || 30 }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5 } })),
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Places par cr\u00E9neau"),
                            React.createElement("input", { type: "number", min: "1", value: f.parallelSlots || 1, onChange: e => upd(f.id, { parallelSlots: +e.target.value || 1 }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5 } })),
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Heure d\u00E9but"),
                            React.createElement("input", { type: "time", value: f.startHour || "08:00", onChange: e => upd(f.id, { startHour: e.target.value }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5 } })),
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, marginBottom: 5 } }, "Heure fin"),
                            React.createElement("input", { type: "time", value: f.endHour || "18:00", onChange: e => upd(f.id, { endHour: e.target.value }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5 } }))),
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm, margin: "10px 0 6px" } }, "Jours disponibles"),
                    React.createElement("div", { style: { display: "flex", gap: 5, flexWrap: "wrap" } }, [['1', 'Lun'], ['2', 'Mar'], ['3', 'Mer'], ['4', 'Jeu'], ['5', 'Ven'], ['6', 'Sam'], ['0', 'Dim']].map(d => { const arr = f.daysAvailable || []; const on = arr.includes(d[0]); return React.createElement("button", { key: d[0], type: "button", onClick: () => upd(f.id, { daysAvailable: on ? arr.filter(x => x !== d[0]) : [...arr, d[0]] }), style: { padding: "5px 10px", borderRadius: 20, border: "1.5px solid " + (on ? "#2563eb" : C.bd), background: on ? "#DBEAFE" : "#fff", color: on ? "#2563eb" : C.tm, fontSize: 11, fontWeight: 800, cursor: "pointer" } }, d[1]); })),
                    React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, padding: "8px 10px", background: "#fff", borderRadius: 8, border: "1px solid " + C.bd } },
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 12, fontWeight: 800 } }, "Validation manuelle"),
                            React.createElement("div", { style: { fontSize: 10.5, color: C.tl } }, "Le rendez-vous sera en attente")),
                        React.createElement(Tog, { on: !!f.manualValidation, cb: () => upd(f.id, { manualValidation: !f.manualValidation }) })))),
                React.createElement("div", null,
                    React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: f.showLeg ? 6 : 0 } },
                        React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm } }, "L\u00E9gende"),
                        React.createElement(Tog, { on: f.showLeg, cb: () => upd(f.id, { showLeg: !f.showLeg }) })),
                    f.showLeg && (React.createElement("textarea", { value: f.leg || "", onChange: e => upd(f.id, { leg: e.target.value }), placeholder: "Texte sous le label...", style: { width: "100%", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "7px 10px", fontSize: 12.5, fontFamily: "inherit", outline: "none", resize: "none", height: 56, boxSizing: "border-box" }, onFocus: e => { e.target.style.borderColor = C.em; }, onBlur: e => { e.target.style.borderColor = C.bd; } }))),
                !["sep", "sign", "photo", "location"].includes(f.type) && (React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" } },
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.tm } }, "Champ duplicable"),
                        React.createElement("div", { style: { fontSize: 11, color: C.ts, marginTop: 1 } }, "L'op\u00E9rateur peut ajouter plusieurs valeurs")),
                    React.createElement(Tog, { on: f.dup, cb: () => upd(f.id, { dup: !f.dup }) }))))),
            tab === "vld" && (React.createElement("div", null,
                (!f.vlds || f.vlds.length === 0) && (React.createElement("div", { style: { textAlign: "center", padding: "18px 0", color: C.ts, fontSize: 12 } }, "Aucun validateur configur\u00E9")),
                (f.vlds || []).map((v, i) => (React.createElement("div", { key: i, style: { display: "flex", gap: 8, alignItems: "center", padding: "8px 10px", border: "1.5px solid " + C.bd, borderRadius: 8, marginBottom: 6, background: C.bg } },
                    React.createElement("span", { style: { width: 20, height: 20, borderRadius: 5, background: C.em, color: "#fff", fontSize: 10, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 } }, i + 1),
                    React.createElement("span", { style: { flex: 1, fontSize: 12.5, fontWeight: 600 } }, v.n),
                    v.v && (React.createElement("input", { value: v.val || "", placeholder: "Valeur", onChange: e => { const n = [...(f.vlds || [])]; n[i] = Object.assign(Object.assign({}, n[i]), { val: e.target.value }); upd(f.id, { vlds: n }); }, style: { width: 70, border: "1.5px solid " + C.bd, borderRadius: 6, padding: "4px 7px", fontSize: 12, fontFamily: "inherit", outline: "none" } })),
                    React.createElement("input", { value: v.msg || "", placeholder: "Message d'erreur", onChange: e => { const n = [...(f.vlds || [])]; n[i] = Object.assign(Object.assign({}, n[i]), { msg: e.target.value }); upd(f.id, { vlds: n }); }, style: { flex: 1, border: "1.5px solid " + C.bd, borderRadius: 6, padding: "4px 7px", fontSize: 12, fontFamily: "inherit", outline: "none" } }),
                    React.createElement("button", { onClick: () => { const n = (f.vlds || []).filter((_, j) => j !== i); upd(f.id, { vlds: n }); }, style: { width: 24, height: 24, border: "1px solid #FCA5A5", borderRadius: 5, background: "#FEF2F2", color: "#EF4444", cursor: "pointer", flexShrink: 0 } }, "\u2715")))),
                React.createElement("div", { style: { marginTop: 8 } },
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.ts, marginBottom: 6, textTransform: "uppercase", letterSpacing: ".5px" } }, "Ajouter"),
                    React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5 } }, getVlds(f.type).map(v => (React.createElement("button", { key: v.n, onClick: () => { const n = [...(f.vlds || []), Object.assign(Object.assign({}, v), { val: "", msg: "" })]; upd(f.id, { vlds: n }); }, style: { padding: "4px 11px", borderRadius: 20, border: "1.5px solid " + C.bd, background: "#fff", color: C.tm, fontSize: 11.5, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }, onMouseEnter: e => { e.currentTarget.style.borderColor = C.em; e.currentTarget.style.color = C.em; e.currentTarget.style.background = C.eml; }, onMouseLeave: e => { e.currentTarget.style.borderColor = C.bd; e.currentTarget.style.color = C.tm; e.currentTarget.style.background = "#fff"; } },
                        "\uFF0B ",
                        v.n))))))),
            tab === "trf" && (React.createElement("div", null,
                (!f.trfs || f.trfs.length === 0) && (React.createElement("div", { style: { textAlign: "center", padding: "18px 0", color: C.ts, fontSize: 12 } }, "Aucun transformateur configur\u00E9")),
                (f.trfs || []).map((tr, i) => (React.createElement("div", { key: i, style: { display: "flex", gap: 8, alignItems: "center", padding: "8px 10px", border: "1.5px solid " + C.bd, borderRadius: 8, marginBottom: 6, background: C.bg } },
                    React.createElement("span", { style: { flex: 1, fontSize: 12, fontWeight: 600 } }, tr.n),
                    /(préfixe|suffixe|premiers|derniers)/.test(tr.n) && (React.createElement("input", { value: tr.param || "", placeholder: "Param\u00E8tre", onChange: e => { const n = [...(f.trfs || [])]; n[i] = Object.assign(Object.assign({}, n[i]), { param: e.target.value }); upd(f.id, { trfs: n }); }, style: { width: 80, border: "1.5px solid " + C.bd, borderRadius: 6, padding: "4px 7px", fontSize: 12, fontFamily: "inherit", outline: "none" } })),
                    React.createElement("button", { onClick: () => { const n = (f.trfs || []).filter((_, j) => j !== i); upd(f.id, { trfs: n }); }, style: { width: 24, height: 24, border: "1px solid #FCA5A5", borderRadius: 5, background: "#FEF2F2", color: "#EF4444", cursor: "pointer", flexShrink: 0 } }, "\u2715")))),
                React.createElement("div", { style: { marginTop: 8 } },
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.ts, marginBottom: 6, textTransform: "uppercase", letterSpacing: ".5px" } }, "Ajouter"),
                    React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5 } }, TRANSFORMERS.map(n => (React.createElement("button", { key: n, onClick: () => { const cur = f.trfs || []; upd(f.id, { trfs: [...cur, { n, param: "" }] }); }, style: { padding: "4px 10px", borderRadius: 20, border: "1.5px solid " + C.bd, background: "#fff", color: C.tm, fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }, onMouseEnter: e => { e.currentTarget.style.borderColor = "#8b5cf6"; e.currentTarget.style.color = "#8b5cf6"; e.currentTarget.style.background = "#f5f3ff"; }, onMouseLeave: e => { e.currentTarget.style.borderColor = C.bd; e.currentTarget.style.color = C.tm; e.currentTarget.style.background = "#fff"; } },
                        "\uFF0B ",
                        n))))))),
            tab === "cnd" && (React.createElement("div", null,
                React.createElement("div", { style: { padding: "9px 12px", background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 8, fontSize: 11.5, color: "#92400E", marginBottom: 12 } },
                    "\u26A1 Ce champ s'affiche si ",
                    React.createElement("strong", null, "toutes"),
                    " les conditions sont remplies."),
                (!f.conds || f.conds.length === 0) && (React.createElement("div", { style: { textAlign: "center", padding: "16px 0", color: C.ts, fontSize: 12 } }, "Aucune condition")),
                (f.conds || []).map((c, i) => (React.createElement("div", { key: i, style: { display: "flex", gap: 6, alignItems: "center", marginBottom: 6, padding: "8px 10px", border: "1.5px solid " + C.bd, borderRadius: 8, background: C.bg } },
                    React.createElement("span", { style: { fontSize: 11, color: C.ts, fontWeight: 700, flexShrink: 0 } }, "Si"),
                    React.createElement("input", { value: c.fn || "", placeholder: "Champ source", onChange: e => { const n = [...(f.conds || [])]; n[i] = Object.assign(Object.assign({}, n[i]), { fn: e.target.value }); upd(f.id, { conds: n }); }, style: { flex: 1, border: "1.5px solid " + C.bd, borderRadius: 6, padding: "5px 8px", fontSize: 12, fontFamily: "inherit", outline: "none" } }),
                    React.createElement("select", { value: c.op || "=", onChange: e => { const n = [...(f.conds || [])]; n[i] = Object.assign(Object.assign({}, n[i]), { op: e.target.value }); upd(f.id, { conds: n }); }, style: { width: 52, border: "1.5px solid " + C.bd, borderRadius: 6, padding: "5px 2px", fontSize: 12, fontFamily: "inherit", outline: "none", background: "#fff" } },
                        React.createElement("option", { value: "=" }, "="),
                        React.createElement("option", { value: "!=" }, "\u2260"),
                        React.createElement("option", { value: "contains" }, "\u220B"),
                        React.createElement("option", { value: "empty" }, "\u2205")),
                    React.createElement("input", { value: c.val || "", placeholder: "Valeur", onChange: e => { const n = [...(f.conds || [])]; n[i] = Object.assign(Object.assign({}, n[i]), { val: e.target.value }); upd(f.id, { conds: n }); }, style: { flex: 1, border: "1.5px solid " + C.bd, borderRadius: 6, padding: "5px 8px", fontSize: 12, fontFamily: "inherit", outline: "none" } }),
                    React.createElement("button", { onClick: () => { const n = (f.conds || []).filter((_, j) => j !== i); upd(f.id, { conds: n }); }, style: { width: 24, height: 24, border: "1px solid #FCA5A5", borderRadius: 5, background: "#FEF2F2", color: "#EF4444", cursor: "pointer", flexShrink: 0 } }, "\u2715")))),
                React.createElement("button", { onClick: () => { const n = [...(f.conds || []), { fn: "", op: "=", val: "" }]; upd(f.id, { conds: n }); }, style: { width: "100%", padding: 7, border: "1.5px dashed " + C.bd, borderRadius: 8, background: "transparent", color: C.tl, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", marginTop: 4 } }, "\uFF0B Ajouter une condition"))),
            tab === "dsp" && (React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } },
                React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 12px", border: "1.5px solid " + C.bd, borderRadius: 8, background: C.bg } },
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 12.5, fontWeight: 700 } }, "\uD83D\uDDA5 Supervision"),
                        React.createElement("div", { style: { fontSize: 11, color: C.ts, marginTop: 1 } }, "Interface web")),
                    React.createElement(Tog, { on: f.vSup !== false, cb: () => upd(f.id, { vSup: !(f.vSup !== false) }) })),
                React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 12px", border: "1.5px solid " + C.bd, borderRadius: 8, background: C.bg } },
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 12.5, fontWeight: 700 } }, "\uD83D\uDCF1 App nomade"),
                        React.createElement("div", { style: { fontSize: 11, color: C.ts, marginTop: 1 } }, "Terminaux terrain")),
                    React.createElement(Tog, { on: f.vNom !== false, cb: () => upd(f.id, { vNom: !(f.vNom !== false) }) })),
                React.createElement("div", { style: { padding: "9px 12px", border: "1px dashed " + C.bd, borderRadius: 8, background: "#FAFAFA" } },
                    React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C.ts, marginBottom: 6 } }, "Visible par les r\u00F4les"),
                    React.createElement("div", { style: { display: "flex", gap: 5, flexWrap: "wrap" } }, (typeof rolesList !== "undefined" && rolesList.length ? rolesList : [{ id: "administrateur", nom: "Administrateur" }, { id: "manager", nom: "Manager" }, { id: "operateur", nom: "Opérateur" }]).map(r => {
                        const rid = r.id || r.nom;
                        const on = (f.roles || []).includes(rid);
                        return (React.createElement("button", { key: rid, onClick: () => { const cur = f.roles || []; upd(f.id, { roles: on ? cur.filter(x => x !== rid) : [...cur, rid] }); }, style: { padding: "4px 11px", borderRadius: 20, border: "1.5px solid " + (on ? C.em : C.bd), background: on ? C.eml : "#fff", color: on ? C.em : C.tm, fontSize: 11.5, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, r.nom || rid));
                    })),
                    React.createElement("div", { style: { fontSize: 10.5, color: C.ts, marginTop: 6 } }, "Vide = tous les r\u00F4les")))))));
}
function AddLine({ afterIndex, onAdd }) { return null; }
function Card({ f, editing, selected, selectionCount, onSelect, onEdit, upd, del, onDS, onDO, onDrop, isDO }) {
    const [hov, setHov] = useState(false);
    const t = TYPES.find(x => x.id === f.type);
    const ac = (f.vlds ? f.vlds.length : 0) + (f.conds ? f.conds.length : 0);
    const borderColor = isDO ? C.em : selected ? C.em : hov ? C.bd : C.bd;
    return (React.createElement("div", { draggable: !editing, onDragStart: (e) => onDS(f.id, e), onDragOver: e => { e.preventDefault(); onDO(f.id); }, onDragLeave: () => onDO(null), onDrop: () => onDrop(f.id), onMouseEnter: () => setHov(true), onMouseLeave: () => setHov(false), onClick: (e) => { if (!editing)
            onSelect(f.id, e); }, style: { background: "#fff", border: "1.5px solid " + borderColor, borderRadius: 14, padding: "14px 16px", marginBottom: 6, cursor: editing ? "default" : "pointer", transition: "border-color .15s, box-shadow .15s, transform .15s", boxShadow: isDO ? "0 0 0 3px " + C.em + "30" : selected ? "0 0 0 3px " + C.em + "18, 0 8px 22px rgba(15,23,42,.06)" : hov ? "0 8px 22px rgba(15,23,42,.06)" : "0 1px 4px rgba(15,23,42,.04)", position: "relative" } },
        selected && selectionCount > 1 && (React.createElement("div", { style: { position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", background: C.em, color: "#fff", borderRadius: 999, padding: "2px 10px", fontSize: 11, fontWeight: 900, boxShadow: "0 6px 14px rgba(37,99,235,.22)" } }, "S\u00E9lection group\u00E9e")),
        editing ? (React.createElement("div", { onClick: e => e.stopPropagation() },
            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 10 } },
                React.createElement("div", { style: { width: 28, height: 28, borderRadius: 7, background: t ? t.clr + "18" : "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 } }, t ? t.icon : "?"),
                React.createElement("input", { value: f.label, onChange: e => upd(f.id, { label: e.target.value }), placeholder: "Nom du champ...", style: { flex: 1, border: "none", borderBottom: "2px solid " + C.em, background: "transparent", fontSize: 13.5, fontWeight: 700, outline: "none", fontFamily: "inherit", color: C.tx, padding: "2px 0" } }),
                React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5 } },
                    React.createElement("span", { style: { fontSize: 12, color: C.tl, fontWeight: 600 } }, "Requis"),
                    React.createElement(Tog, { on: f.req, cb: () => upd(f.id, { req: !f.req }) })),
                React.createElement("button", { onClick: () => onEdit(null), style: { width: 26, height: 26, borderRadius: 6, border: "1.5px solid " + C.bd, background: "#fff", cursor: "pointer", fontSize: 11, color: C.tl } }, "\u2713")),
            ["select", "multi"].includes(f.type) && (React.createElement(OptsEd, { opts: f.opts || [], onChange: opts => upd(f.id, { opts }) })),
            React.createElement(Preview, { f: f }),
            React.createElement("div", { style: { marginTop: 8, padding: "7px 10px", borderRadius: 8, border: "1px dashed " + C.bd, background: C.bg, color: C.ts, fontSize: 11.5, lineHeight: 1.35 } }, "Les param\u00E8tres d\u00E9taill\u00E9s du champ se r\u00E8glent dans le panneau de droite."))) : (React.createElement("div", null,
            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: f.type === "sep" ? 0 : 8 } },
                React.createElement("span", { title: "D\u00E9placer", style: { color: C.ts, fontSize: 18, lineHeight: 1, cursor: "grab" } }, "\u283F"),
                React.createElement("span", { style: { fontSize: 13.5, fontWeight: 800, color: C.tx, flex: 1 } },
                    f.label,
                    f.req && (React.createElement("span", { style: { color: C.em } }, " *")),
                    ac > 0 && (React.createElement("span", { title: ac + " paramètre(s)", style: { marginLeft: 6, padding: "1px 6px", borderRadius: 10, background: C.eml, color: C.em, fontSize: 10, fontWeight: 800, border: "1px solid " + C.em2 } },
                        "\u2699 ",
                        ac))),
                React.createElement(Badge, { tid: f.type, xs: true }),
                React.createElement("div", { style: { display: "flex", gap: 3, opacity: hov || selected ? 1 : 0, transition: "opacity .15s" }, onClick: e => e.stopPropagation() },
                    React.createElement("button", { onClick: e => { e.stopPropagation(); onEdit(f.id); }, style: { width: 26, height: 26, borderRadius: 6, border: "1.5px solid " + C.bd, background: "#fff", cursor: "pointer", fontSize: 12, color: C.tl } }, "\u270F\uFE0F"),
                    React.createElement("button", { onClick: e => { e.stopPropagation(); del(f.id); }, style: { width: 26, height: 26, borderRadius: 6, border: "1px solid #FCA5A5", background: "#FEF2F2", color: "#EF4444", cursor: "pointer", flexShrink: 0, fontSize: 11 } }, "\u2715"))),
            f.type !== "sep" && (React.createElement(Preview, { f: f })),
            f.showLeg && f.leg && (React.createElement("div", { style: { fontSize: 11, color: C.tl, marginTop: 5, fontStyle: "italic" } }, f.leg))))));
}
...
function PicoBuilderApp({ initialFields, initialName, initialMeta, onSave }) {
    ...
    const setTrigger = (key, patch) => setTriggers(prev => (Object.assign(Object.assign({}, prev), { [key]: Object.assign(Object.assign({}, (prev[key] || {})), patch) })));
    const mailFields = fields.filter(f => !["sep", "titre", "image", "groupe"].includes(f.type));
    const mailFieldOptions = mailFields.map(f => (React.createElement("option", { key: f.id, value: f.id }, f.label)));
    const insertMailVar = (target, value) => {
        const cur = triggers.sendMail || {};
        setTrigger("sendMail", { [target]: (cur[target] || "") + value });
    };
    const mailVariableButtons = (React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 5, marginTop: 6 } },
        React.createElement("button", { type: "button", onClick: () => insertMailVar("subject", "{formName}"), style: { padding: "4px 9px", border: "1px solid " + C.bd, borderRadius: 20, background: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" } }, "Formulaire"),
        React.createElement("button", { type: "button", onClick: () => insertMailVar("body", "{date_saisie}"), style: { padding: "4px 9px", border: "1px solid " + C.bd, borderRadius: 20, background: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" } }, "Date saisie"),
        React.createElement("button", { type: "button", onClick: () => insertMailVar("body", "{utilisateur}"), style: { padding: "4px 9px", border: "1px solid " + C.bd, borderRadius: 20, background: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" } }, "Utilisateur"),
        React.createElement("button", { type: "button", onClick: () => insertMailVar("body", "{all_fields}"), style: { padding: "4px 9px", border: "1px solid " + C.bd, borderRadius: 20, background: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" } }, "Tous les champs"),
        mailFields.map(f => (React.createElement("button", { key: f.id, type: "button", onClick: () => insertMailVar("body", "{champ:" + (f.key || slugKey(f.label)) + "}"), style: { padding: "4px 9px", border: "1px solid " + C.em2, borderRadius: 20, background: C.eml, color: C.em, fontSize: 11, fontWeight: 800, cursor: "pointer" } }, f.label)))));
    ...
    const mailFieldChips = (target) => (React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6, marginTop: 6 } }, mailFields.map(f => {
        const cur = triggers.sendMail || {};
        const arr = Array.isArray(cur[target]) ? cur[target] : [];
        const on = arr.includes(f.id);
        return (React.createElement("button", { key: f.id, type: "button", onClick: () => toggleMailField(target, f.id), style: { padding: "5px 10px", borderRadius: 999, border: "1.5px solid " + (on ? C.em : C.bd), background: on ? C.eml : "#fff", color: on ? C.em : C.tm, fontSize: 11, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" } },
            on ? "✓ " : "+ ",
            f.label));
    })));
    const dbList = (typeof DATABASES_DATA !== "undefined" ? DATABASES_DATA : []);
    const selectedDb = dbList.find(db => { var _a; return String(db.id) === String((_a = triggers.addDbRow) === null || _a === void 0 ? void 0 : _a.targetDbId); });
    ...
    const dbFieldOptions = (React.createElement(React.Fragment, null,
        React.createElement("option", { value: "" }, "\u2014 Aucun champ \u2014"),
        mailFields.map(f => (React.createElement("option", { key: f.id, value: f.id }, f.label)))));
    ...
    const selectField = useCallback((id, e) => {
        ...
        setFields(prev => {
            ...
            if (dir < 0) {
                for (let i = 1; i < a.length; i++) {
                    if (ids.has(a[i].id) && !ids.has(a[i - 1].id)) {
                        const tmp = a[i - 1];
                        a[i - 1] = a[i];
                        a[i] = tmp;
                    }
    ...
    const duplicateSelected = useCallback(() => {
        setFields(prev => {
            ...
            if (!ids.size)
                return prev;
            const copies = prev.filter(f => ids.has(f.id)).map(f => (Object.assign(Object.assign({}, JSON.parse(JSON.stringify(f))), { id: nid.current++, label: (f.label || 'Champ') + ' (copie)' })));
            const last = Math.max(...prev.map((f, i) => ids.has(f.id) ? i : -1));
            const next = [...prev];
            next.splice(last + 1, 0, ...copies);
            setSelectedIds(copies.map(f => f.id));
            setEditId(((_a = copies[0]) === null || _a === void 0 ? void 0 : _a.id) || null);
            return next;
        });
    }, [selectedIds]);
    ...
    const addF = useCallback((ai, type, label) => {
        ...
        const f = Object.assign({ id: nid.current++, type, req: false, label: (label && label.trim()) ? label.trim() : t.label, opts: ["select", "multi"].includes(type) ? ["Option 1", "Option 2"] : [], ph: "", leg: "", showLeg: false, vSup: true, vNom: true, dup: false, vlds: [], conds: [] }, (type === "appointment" ? { slotDuration: 30, startHour: "08:00", endHour: "18:00", parallelSlots: 2, daysAvailable: ["1", "2", "3", "4", "5"], manualValidation: false } : {}));
        setFields(prev => { const next = [...prev]; next.splice(ai + 1, 0, f); return next; });
        setEditId(f.id);
        setSelectedIds([f.id]);
    }, []);
    const updF = useCallback((id, u) => { setFields(prev => prev.map(f => f.id === id ? Object.assign(Object.assign({}, f), u) : f)); }, []);
    const delF = useCallback((id) => { setFields(prev => prev.filter(f => f.id !== id)); setEditId(e => e === id ? null : e); setSelectedIds(prev => prev.filter(x => x !== id)); }, []);
    const drop = useCallback((tid) => {
        if (!dragId || dragId === tid)
            return;
        setFields(prev => {
            const movingIds = (selectedIds.includes(dragId) ? selectedIds : [dragId]);
            if (movingIds.includes(tid))
                return prev;
            const movingSet = new Set(movingIds);
            const moving = prev.filter(f => movingSet.has(f.id));
            const rest = prev.filter(f => !movingSet.has(f.id));
            const ti = rest.findIndex(f => f.id === tid);
            if (ti < 0)
                return prev;
            const next = [...rest];
            next.splice(ti, 0, ...moving);
            return next;
        });
        setDragId(null);
        setDragOvId(null);
    ...
    const selectedField = fields.find(f => f.id === editId) || fields.find(f => selectedIds.includes(f.id)) || null;
    const handleSave = () => {
        setSaved(true);
        setTimeout(() => setSaved(false), 2200);
        if (onSave)
            onSave(fname, fields.map(f => ({
    ...
    return (React.createElement("div", { style: { flex: 1, minHeight: 0, background: C.bg, fontFamily: "'DM Sans',-apple-system,sans-serif", fontSize: 13, color: C.tx, display: "flex", flexDirection: "column", overflow: "hidden" } },
        React.createElement("div", { style: { height: 64, background: "#fff", borderBottom: "1px solid " + C.bd, display: "flex", alignItems: "center", padding: "0 18px", gap: 10, boxShadow: "0 1px 3px rgba(0,0,0,.04)", flexShrink: 0 } },
            React.createElement("button", { onClick: () => { if (window.goList)
                    window.goList(); }, style: { display: "flex", alignItems: "center", gap: 5, padding: "5px 11px", borderRadius: 7, border: "1.5px solid " + C.bd, background: "#fff", color: C.tl, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" } }, "\u2190 Formulaires"),
            React.createElement("div", { style: { width: 1, height: 20, background: C.bd } }),
            editFn ? (React.createElement("input", { value: fname, autoFocus: true, onChange: e => setFname(e.target.value), onBlur: () => setEditFn(false), onKeyDown: e => { if (e.key === "Enter")
                    setEditFn(false); }, style: { fontSize: 22, fontWeight: 900, border: "none", borderBottom: "2px solid " + C.em, background: "transparent", outline: "none", color: C.tx, fontFamily: "inherit", minWidth: 200 } })) : (React.createElement("div", { onClick: () => setEditFn(true), style: { fontSize: 14, fontWeight: 800, cursor: "text", display: "flex", alignItems: "center", gap: 6 } },
                fname,
                React.createElement("span", { style: { color: C.bd, fontSize: 12, fontWeight: 400 } }, "\u270E"))),
            React.createElement("div", { style: { flex: 1 } }),
            React.createElement("span", { style: { fontSize: 12, color: saved ? C.em : C.ts, fontWeight: 600, transition: "color .3s" } }, saved ? "✓ Enregistré" : fields.length + " champs · " + req + " requis"),
            React.createElement("button", { style: { padding: "6px 13px", borderRadius: 8, border: "1.5px solid " + C.bd, background: "#fff", color: C.tm, fontSize: 12.5, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, "\uD83D\uDC41 Aper\u00E7u"),
            React.createElement("button", { onClick: handleSave, style: { padding: "6px 16px", borderRadius: 8, border: "none", background: C.em, color: "#fff", fontSize: 12.5, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, "\uD83D\uDCBE Enregistrer")),
        React.createElement("div", { style: { background: "#fff", borderBottom: "1px solid " + C.bd, flexShrink: 0 } },
            React.createElement("button", { onClick: () => setShowSettings(!showSettings), style: { width: "100%", border: "none", background: "#fff", padding: "10px 18px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontFamily: "inherit", color: C.tx } },
                React.createElement("span", { style: { width: 22, height: 22, borderRadius: 7, background: C.eml, color: C.em, display: "inline-flex", alignItems: "center", justifyContent: "center", fontWeight: 900 } }, "\u2699"),
                React.createElement("span", { style: { fontSize: 13, fontWeight: 800 } }, "Param\u00E9trage du formulaire"),
                React.createElement("span", { style: { fontSize: 12, color: C.ts } }, "nom \u00B7 r\u00F4les visibles \u00B7 actions automatiques apr\u00E8s validation"),
                React.createElement("span", { style: { marginLeft: "auto", fontSize: 11, color: C.ts, transform: showSettings ? "rotate(180deg)" : "none", display: "inline-block" } }, "\u25BE")),
            showSettings && (React.createElement("div", { style: { padding: "0 18px 16px", display: "grid", gridTemplateColumns: "1fr 1fr 1.2fr", gap: 12 } },
                React.createElement("div", { style: { border: "1.5px solid " + C.bd, borderRadius: 12, padding: 12, background: C.bg } },
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 800, marginBottom: 8 } }, "Identit\u00E9"),
                    React.createElement("label", { style: { fontSize: 11, color: C.tl, fontWeight: 700 } }, "Nom du formulaire"),
                    React.createElement("input", { value: fname, onChange: e => setFname(e.target.value), placeholder: "Ex : Arrivage CNPE Blaye", style: { marginTop: 5, width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 8, padding: "9px 10px", fontFamily: "inherit", fontSize: 13 } })),
                React.createElement("div", { style: { border: "1.5px solid " + C.bd, borderRadius: 12, padding: 12, background: C.bg } },
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 800, marginBottom: 8 } }, "Visibilit\u00E9 par r\u00F4le"),
                    React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 7 } },
                        rolesList.map(r => (React.createElement("label", { key: r.id, style: { display: "flex", alignItems: "center", gap: 8, fontSize: 12, cursor: "pointer" } },
                            React.createElement("input", { type: "checkbox", checked: visibleRoles.includes(r.id), onChange: () => toggleRole(r.id) }),
                            React.createElement("span", { style: { fontWeight: 700 } }, r.nom)))),
                        rolesList.length === 0 && React.createElement("span", { style: { fontSize: 12, color: C.ts } }, "Aucun r\u00F4le configur\u00E9."))),
                React.createElement("div", { style: { border: "1.5px solid " + C.bd, borderRadius: 12, padding: 12, background: C.bg, maxHeight: "520px", overflowY: "scroll", overflowX: "hidden" } },
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 800, marginBottom: 8 } }, "Actions apr\u00E8s validation"),
                    React.createElement("div", { style: { display: "grid", gap: 8 } },
                        React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 700 } },
                            React.createElement("input", { type: "checkbox", checked: !!((_a = triggers.printLabel) === null || _a === void 0 ? void 0 : _a.enabled), onChange: e => setTrigger('printLabel', { enabled: e.target.checked }) }),
                            " Imprimer une \u00E9tiquette"),
                        ((_b = triggers.printLabel) === null || _b === void 0 ? void 0 : _b.enabled) && React.createElement("div", { style: { display: "flex", gap: 6 } },
                            React.createElement("input", { value: triggers.printLabel.template || "", onChange: e => setTrigger('printLabel', { template: e.target.value }), placeholder: "Mod\u00E8le d'\u00E9tiquette", style: { flex: 1, border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } }),
                            React.createElement("input", { value: triggers.printLabel.printer || "", onChange: e => setTrigger('printLabel', { printer: e.target.value }), placeholder: "Imprimante", style: { flex: 1, border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } })),
                        React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 700 } },
                            React.createElement("input", { type: "checkbox", checked: !!((_c = triggers.sendMail) === null || _c === void 0 ? void 0 : _c.enabled), onChange: e => setTrigger('sendMail', { enabled: e.target.checked }) }),
                            " Envoyer un mail"),
                        ((_d = triggers.sendMail) === null || _d === void 0 ? void 0 : _d.enabled) && (React.createElement("div", { style: { display: "grid", gap: 8, background: "#fff", border: "1px solid " + C.bd, borderRadius: 10, padding: 10 } },
                            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 } },
                                React.createElement("select", { value: triggers.sendMail.event || "create", onChange: e => setTrigger('sendMail', { event: e.target.value }), style: { border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } },
                                    React.createElement("option", { value: "create" }, "\u00C0 la cr\u00E9ation"),
                                    React.createElement("option", { value: "update" }, "\u00C0 la modification")),
                                React.createElement("select", { value: triggers.sendMail.conditionOperator || "always", onChange: e => setTrigger('sendMail', { conditionOperator: e.target.value }), style: { border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } },
                                    React.createElement("option", { value: "always" }, "Toujours envoyer"),
                                    React.createElement("option", { value: "equals" }, "Si champ = valeur"),
                                    React.createElement("option", { value: "not_equals" }, "Si champ \u2260 valeur"),
                                    React.createElement("option", { value: "contains" }, "Si champ contient"),
                                    React.createElement("option", { value: "not_empty" }, "Si champ non vide"))),
                            triggers.sendMail.conditionOperator !== "always" && (React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 } },
                                React.createElement("select", { value: triggers.sendMail.conditionField || "", onChange: e => setTrigger('sendMail', { conditionField: e.target.value }), style: { border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } },
                                    React.createElement("option", { value: "" }, "Champ condition"),
                                    mailFieldOptions),
                                React.createElement("input", { value: triggers.sendMail.conditionValue || "", onChange: e => setTrigger('sendMail', { conditionValue: e.target.value }), placeholder: "Valeur attendue", style: { border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } }))),
                            React.createElement("div", null,
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 800, color: C.tl, marginBottom: 5 } }, "Destinataires fixes"),
                                React.createElement("input", { value: triggers.sendMail.to || "", onChange: e => setTrigger('sendMail', { to: e.target.value }), placeholder: "Emails s\u00E9par\u00E9s par ;", style: { width: "100%", boxSizing: "border-box", border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } })),
                            React.createElement("div", null,
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 800, color: C.tl, marginBottom: 5 } }, "Destinataires depuis les champs"),
                                mailFieldChips("toFields")),
                            React.createElement("div", null,
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 800, color: C.tl, marginBottom: 5 } }, "CC fixes"),
                                React.createElement("input", { value: triggers.sendMail.cc || "", onChange: e => setTrigger('sendMail', { cc: e.target.value }), placeholder: "CC s\u00E9par\u00E9s par ;", style: { width: "100%", boxSizing: "border-box", border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } })),
                            React.createElement("div", null,
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 800, color: C.tl, marginBottom: 5 } }, "CC depuis les champs"),
                                mailFieldChips("ccFields")),
                            React.createElement("input", { value: triggers.sendMail.subject || "", onChange: e => setTrigger('sendMail', { subject: e.target.value }), placeholder: "Objet du mail", style: { border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } }),
                            React.createElement("div", null,
                                React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5 } },
                                    React.createElement("span", { style: { fontSize: 11, fontWeight: 800, color: C.tl } }, "Message"),
                                    React.createElement("span", { style: { fontSize: 10, color: C.ts } }, "Cliquez sur une variable pour l'ins\u00E9rer")),
                                React.createElement("textarea", { value: triggers.sendMail.body || "", onChange: e => setTrigger('sendMail', { body: e.target.value }), placeholder: "Message du mail. Variable recommand\u00E9e : {all_fields}", style: { border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12, minHeight: 74, fontFamily: "inherit", resize: "vertical", width: "100%", boxSizing: "border-box" } }),
                                mailVariableButtons),
                            React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 700 } },
                                React.createElement("input", { type: "checkbox", checked: triggers.sendMail.attachPdf !== false, onChange: e => setTrigger('sendMail', { attachPdf: e.target.checked }) }),
                                "Joindre automatiquement le PDF de la saisie au mail"),
                            React.createElement("div", { style: { fontSize: 11.5, color: C.tl, lineHeight: 1.4, background: C.bg, border: "1px dashed " + C.bd, borderRadius: 8, padding: 9 } },
                                "Le mail reprend le logo via ",
                                React.createElement("b", null, "PICOTRACK_LOGO_URL"),
                                ". Le PDF est g\u00E9n\u00E9r\u00E9 automatiquement avec les champs du formulaire, la date, l\u2019utilisateur et la r\u00E9f\u00E9rence de saisie."))),
                        React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 700 } },
                            React.createElement("input", { type: "checkbox", checked: !!((_e = triggers.addDbRow) === null || _e === void 0 ? void 0 : _e.enabled), onChange: e => setTrigger('addDbRow', { enabled: e.target.checked }) }),
                            " Ajouter une ligne dans une base"),
                        ((_f = triggers.addDbRow) === null || _f === void 0 ? void 0 : _f.enabled) && (React.createElement("div", { style: { display: "grid", gap: 10, background: "#fff", border: "1px solid " + C.bd, borderRadius: 10, padding: 10 } },
                            React.createElement("div", null,
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 800, color: C.tl, marginBottom: 5 } }, "Base cible"),
                                React.createElement("select", { value: triggers.addDbRow.targetDbId || "", onChange: e => setTrigger('addDbRow', { targetDbId: e.target.value, mappings: [] }), style: { width: "100%", border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } },
                                    React.createElement("option", { value: "" }, "\u2014 S\u00E9lectionner une base \u2014"),
                                    dbList.map(db => (React.createElement("option", { key: db.id, value: db.id }, db.nom))))),
                            React.createElement("div", null,
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 800, color: C.tl, marginBottom: 5 } }, "Mode de mapping"),
                                React.createElement("select", { value: triggers.addDbRow.mappingMode || "auto", onChange: e => setTrigger('addDbRow', { mappingMode: e.target.value }), style: { width: "100%", border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } },
                                    React.createElement("option", { value: "auto" }, "Mapping auto \u2014 noms identiques"),
                                    React.createElement("option", { value: "manual" }, "Mapping manuel \u2014 champ par colonne"))),
                            selectedDb && triggers.addDbRow.mappingMode === "manual" && (React.createElement("div", { style: { display: "grid", gap: 8 } },
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 900, color: C.tl, textTransform: "uppercase", letterSpacing: ".5px" } }, "Mapping manuel"),
                                (selectedDb.columns || []).map(col => (React.createElement("div", { key: col.id, style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, alignItems: "center" } },
                                    React.createElement("div", { style: { fontSize: 12, fontWeight: 800, color: C.tm, background: C.bg, border: "1px solid " + C.bd, borderRadius: 7, padding: "7px 8px" } },
                                        "Colonne : ",
                                        col.nom),
                                    React.createElement("select", { value: getMappedField(col.id), onChange: e => setDbMapping(col.id, e.target.value), style: { border: "1px solid " + C.bd, borderRadius: 7, padding: 7, fontSize: 12 } }, dbFieldOptions)))))),
                            selectedDb && triggers.addDbRow.mappingMode === "auto" && (React.createElement("div", { style: { fontSize: 11.5, color: C.tl, lineHeight: 1.4, background: C.bg, border: "1px dashed " + C.bd, borderRadius: 8, padding: 9 } }, "Mapping auto : PicoTrack remplira les colonnes dont le nom correspond \u00E0 un champ du formulaire.")),
                            !selectedDb && (React.createElement("div", { style: { fontSize: 11.5, color: C.ts, lineHeight: 1.4 } }, "Cr\u00E9e d'abord une base dans Database Studio si aucune base n'appara\u00EEt."))))))))),
        React.createElement("div", { style: { flex: 1, minHeight: 0, display: "grid", gridTemplateColumns: "270px minmax(520px,1fr) 360px", overflow: "hidden", background: C.bg } },
            React.createElement("aside", { style: { borderRight: "1px solid " + C.bd, background: "#fff", padding: 18, overflowY: "auto" } },
                React.createElement("div", { style: { fontSize: 11, fontWeight: 900, color: C.ts, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 } }, "Biblioth\u00E8que de blocs"),
                TYPE_CATS.map(cat => (React.createElement("div", { key: cat.id, style: { marginBottom: 14 } },
                    React.createElement("div", { style: { fontSize: 10, fontWeight: 900, color: C.tl, textTransform: "uppercase", letterSpacing: .7, margin: "0 0 7px" } }, cat.label),
                    React.createElement("div", { style: { display: "grid", gap: 6 } }, TYPES.filter(t => t.cat === cat.id).map(t => (React.createElement("button", { key: t.id, onClick: () => addF(fields.length - 1, t.id, ""), style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 9px", border: "1.5px solid " + C.bd, borderRadius: 10, background: "#fff", cursor: "pointer", fontFamily: "inherit", color: C.tm, fontWeight: 800, fontSize: 12, textAlign: "left" } },
                        React.createElement("span", { style: { width: 24, height: 24, borderRadius: 8, background: t.clr + "18", color: t.clr, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0 } }, t.icon),
                        React.createElement("span", null, t.label))))))))),
            React.createElement("div", { style: { overflowY: "auto", padding: "20px 0 60px" } },
                React.createElement("div", { style: { maxWidth: 860, margin: "0 auto", padding: "0 18px" } },
                    React.createElement("div", { style: { background: "#fff", border: "1.5px solid " + C.bd, borderRadius: 18, padding: "18px 22px", marginBottom: 8, boxShadow: "0 1px 6px rgba(0,0,0,.05)" } },
                        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 11 } },
                            React.createElement("div", { style: { width: 4, height: 36, borderRadius: 3, background: C.em, flexShrink: 0 } }),
                            React.createElement("div", { style: { flex: 1 } },
                                React.createElement("div", { style: { fontSize: 15, fontWeight: 800 } }, fname),
                                React.createElement("div", { style: { fontSize: 11.5, color: C.ts, marginTop: 2 } },
                                    req,
                                    " obligatoire",
                                    req !== 1 ? "s" : "",
                                    " \u00B7 D\u00E9ployable PC \u2194 PAD")))),
                    React.createElement(AddLine, { afterIndex: -1, onAdd: addF }),
                    fields.map((f, idx) => (React.createElement("div", { key: f.id },
                        React.createElement(Card, { f: f, editing: editId === f.id, selected: selectedIds.includes(f.id), selectionCount: selectedIds.length, onSelect: selectField, onEdit: (id) => { setEditId(id); if (id)
                                setSelectedIds([id]); }, upd: updF, del: delF, onDS: (id) => { setDragId(id); if (!selectedIds.includes(id))
                                setSelectedIds([id]); setEditId(null); }, onDO: setDragOvId, onDrop: drop, isDO: dragOvId === f.id && dragId !== f.id }),
                        React.createElement(AddLine, { afterIndex: idx, onAdd: addF })))),
                    fields.length === 0 && (React.createElement("div", { style: { border: "2px dashed " + C.bd, borderRadius: 12, padding: "40px 20px", textAlign: "center", color: C.ts, background: "#fff" } },
                        React.createElement("div", { style: { fontSize: 28, marginBottom: 8, opacity: .4 } }, "+"),
                        React.createElement("div", { style: { fontWeight: 600, fontSize: 14 } }, "Ajoutez un premier bloc depuis la biblioth\u00E8que"))),
                    fields.length > 0 && (React.createElement("div", { style: { textAlign: "center", marginTop: 12, color: C.ts, fontSize: 12 } },
                        fields.length,
                        " champ",
                        fields.length !== 1 ? "s" : "",
                        " \u00B7 ",
                        ...
                        " obligatoire",
                        req !== 1 ? "s" : "")),
                    selectedIds.length > 1 && (React.createElement("div", { style: { position: "sticky", bottom: 12, margin: "18px auto 0", background: "#fff", border: "1.5px solid " + C.bd, borderRadius: 16, padding: "12px 16px", boxShadow: "0 18px 45px rgba(15,23,42,.14)", display: "flex", alignItems: "center", gap: 12, maxWidth: 680, zIndex: 5 } },
                        React.createElement("div", { style: { fontSize: 13, fontWeight: 900, flex: 1 } },
                            selectedIds.length,
                            " champs s\u00E9lectionn\u00E9s"),
                        React.createElement("button", { onClick: () => moveSelected(-1), style: { padding: "8px 12px", border: "1.5px solid " + C.em, borderRadius: 10, background: C.eml, color: C.em, fontWeight: 900, cursor: "pointer" } }, "D\u00E9placer \u2191"),
                        React.createElement("button", { onClick: () => moveSelected(1), style: { padding: "8px 12px", border: "1.5px solid " + C.em, borderRadius: 10, background: C.eml, color: C.em, fontWeight: 900, cursor: "pointer" } }, "D\u00E9placer \u2193"),
                        React.createElement("button", { onClick: duplicateSelected, style: { padding: "8px 12px", border: "1.5px solid " + C.bd, borderRadius: 10, background: "#fff", color: C.tm, fontWeight: 900, cursor: "pointer" } }, "Dupliquer"),
                        React.createElement("button", { onClick: deleteSelected, style: { padding: "8px 12px", border: "1.5px solid #FCA5A5", borderRadius: 10, background: C.dangerL, color: C.danger, fontWeight: 900, cursor: "pointer" } }, "Supprimer"))))),
            React.createElement("aside", { style: { borderLeft: "1px solid " + C.bd, background: "#fff", padding: 18, overflowY: "auto" } },
                React.createElement("div", { style: { fontSize: 11, fontWeight: 900, color: C.ts, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 } }, "Param\u00E8tres du champ"),
                selectedField ? (React.createElement("div", { style: { display: "grid", gap: 10 } },
                    React.createElement("div", { style: { padding: 12, border: "1.5px solid " + C.bd, borderRadius: 12, background: C.bg } },
                        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8 } },
                            React.createElement("div", { style: { fontSize: 12, fontWeight: 900, flex: 1 } }, selectedField.label),
                            React.createElement(Badge, { tid: selectedField.type, xs: true })),
                        React.createElement("label", { style: { fontSize: 11, fontWeight: 800, color: C.tl, display: "block", marginBottom: 5 } }, "Libell\u00E9 affich\u00E9"),
                        React.createElement("input", { value: selectedField.label || "", onChange: e => updF(selectedField.id, { label: e.target.value }), style: { width: "100%", boxSizing: "border-box", border: "1.5px solid " + C.bd, borderRadius: 9, padding: "8px 10px", fontFamily: "inherit", fontSize: 12, marginBottom: 9 } }),
                        React.createElement("label", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 12, fontWeight: 800, color: C.tm } },
                            "Obligatoire ",
                            React.createElement(Tog, { on: selectedField.req, cb: () => updF(selectedField.id, { req: !selectedField.req }) }))),
                    React.createElement("div", { style: { padding: 12, border: "1.5px solid " + C.bd, borderRadius: 12, background: "#fff" } },
                        React.createElement("div", { style: { fontSize: 11, fontWeight: 900, color: C.ts, textTransform: "uppercase", letterSpacing: .8, marginBottom: 8 } }, "Type de champ"),
                        React.createElement(TypePicker, { active: selectedField.type, onSel: type => updF(selectedField.id, Object.assign({ type, opts: ['select', 'multi'].includes(type) ? ['Option 1', 'Option 2'] : [] }, (type === "appointment" ? { slotDuration: selectedField.slotDuration || 30, startHour: selectedField.startHour || "08:00", endHour: selectedField.endHour || "18:00", parallelSlots: selectedField.parallelSlots || 2, daysAvailable: selectedField.daysAvailable || ["1", "2", "3", "4", "5"], manualValidation: !!selectedField.manualValidation } : {}))) })),
                    React.createElement(Adv, { f: selectedField, upd: updF }))) : (React.createElement("div", { style: { padding: 18, border: "1.5px dashed " + C.bd, borderRadius: 12, textAlign: "center", color: C.ts, fontSize: 12 } }, "S\u00E9lectionnez un champ dans le formulaire pour modifier ses param\u00E8tres."))))));
}
...
function _padVisibleForms(){
  ...
  return rows.filter(f => f && f.actif !== false && f.published === true && (typeof _ptCanSeeByRoles !== 'function' || _ptCanSeeByRoles(f.visibleRoles || f.visible_roles || [])));
}
