  ...
  setTimeout(function(){
    mountReactBuilder(curForm, function(nom, fields, meta){
      ...
      if (typeof DB !== 'undefined') {
        const savePromise = curForm ? DB.updateForm(data.id, data) : DB.createForm(data);
        savePromise.then(function(){ console.log('[DB] Formulaire enregistré'); })
          .catch(function(e){ console.warn('[DB] Erreur formulaire:', e.message); });
      }
...
function saveForm(quit){
  ...
  if (typeof DB !== 'undefined') {
    const savePromise = curForm ? DB.updateForm(data.id, data) : DB.createForm(data);
    savePromise.then(() => console.log('[DB] Formulaire enregistré'))
      .catch(e => console.warn('[DB] Erreur formulaire:', e.message));
  }
