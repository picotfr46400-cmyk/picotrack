...
function saveService(quit) {
  ...
  const wasExistingService = !!curService;
  const data = {
  ...
  // Sauvegarde Supabase : PC -> PAD et conservation après actualisation
  if (typeof DB !== 'undefined') {
    const savePromise = wasExistingService ? DB.updateService(data.id, data) : DB.createService(data);
    savePromise
      .then(() => console.log('[DB] Service enregistré'))
      .catch(e => console.warn('[DB] Erreur service:', e.message));
  }
...
function submitServiceInstance(f, svc) {
  ...
  // Sauvegarde Supabase : demande service PAD/PC -> base centrale
  if (typeof DB !== 'undefined') {
    const device = (typeof isPadMode === 'function' && isPadMode()) ? 'pad' : 'desktop';
    DB.createInstance(mapLocalInstanceToDb(newInst, {...saisieValues}, device))
      .then(() => console.log('[DB] Demande service enregistrée'))
      .catch(e => console.warn('[DB] Erreur demande service:', e.message));
  }
