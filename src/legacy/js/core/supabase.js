// PicoTrack — client applicatif sécurisé par proxy serveur.
// Aucune URL Supabase ni clé anon n'est chargée directement par le navigateur.
...
const PT_SESSION_STORAGE_KEY = 'pt_auth_session_proxy';

function _readProxySession() {
  try { return JSON.parse(localStorage.getItem(PT_SESSION_STORAGE_KEY) || 'null'); } catch (_) { return null; }
}
function _writeProxySession(session) {
  if (session) localStorage.setItem(PT_SESSION_STORAGE_KEY, JSON.stringify(session));
  else localStorage.removeItem(PT_SESSION_STORAGE_KEY);
}
function _getSupaConfig() { return { url: '', key: '' }; }
...
async function _getAuthHeader() {
  const session = _readProxySession();
...
function _encodePath(path) {
  try { return btoa(unescape(encodeURIComponent(String(path || '')))); } catch (_) { return btoa(String(path || '')); }
}
async function sbFetch(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    updateSupabaseStatusUI('offline', `Données ${res.status}`);
    throw new Error(`Données ${res.status}: ${text}`);
  }
...
async function sbFunction(functionName, payload = {}) {
  ...
  if (!res.ok) throw new Error(json?.error || json?.message || `Function ${functionName} ${res.status}`);
  return json;
}
...
async function ptSignIn(email, password) {
  ...
  if (!res.ok) throw new Error(json?.error || json?.message || 'Connexion impossible');
  _writeProxySession(json.session || json);
  return { session: json.session || json, user: (json.session || json)?.user };
}
async function ptSignOut() {
  const token = await _getAuthHeader();
  try { await fetch('/api/auth', { method:'POST', headers:{ 'Content-Type':'application/json', ...(token ? {Authorization:`Bearer ${token}`} : {}) }, body: JSON.stringify({ action:'signOut' }) }); } catch(_){ }
  _writeProxySession(null);
...
async function ptGetCurrentUser() {
  ...
  try {
    ...
    const rows = await sbFetch(`user_profiles?id=eq.${uid}&select=*&limit=1`);
