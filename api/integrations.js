...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const action = cleanString(body.action || 'load', 40);
    const env = envFrom(profile, body.environment_code || body.env);

    if (action === 'load') {
      const row = await loadRow(req, env);
      return json(res, 200, { ok: true, environment_code: env, persisted: !!row, ...publicState(parseStoredConfig(row)) });
    }
    ...
    if (action === 'save') {
      ...
      return json(res, 200, { ok: true, environment_code: env, persisted: true, ...publicState(next) });
    }

    if (action === 'create_key') {
      const current = parseStoredConfig(await loadRow(req, env));
      if (safeArray(current.keys).length >= MAX_KEYS) {
        return json(res, 400, { error: 'Nombre maximum de clés atteint.' });
      }
    ...
    if (action === 'test_webhook' || action === 'dispatch') {
      ...
      if (!result.ok) {
        return json(res, 502, {
          ok: false,
          error: result.error || `POST webhook HTTP ${result.status}`,
    ...
    return json(res, err.status && err.status >= 400 ? err.status : 500, { error: err.message || 'Erreur API intégrations' });
  }
};
